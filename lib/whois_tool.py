...
def lookup(target):
    ...
    if not server:
        return {"target": target, "error": "No WHOIS server found for TLD", "raw": ""}
    ...
    # Follow a referral if the registry points us at the registrar.
    referral = re.search(r"(?:Registrar WHOIS Server|ReferralServer):\s*(?:whois://)?(\S+)", raw)
    if referral:
        try:
            raw2 = _query(referral.group(1), target)
            raw = raw + "\n\n%% Referral follow-up %%\n\n" + raw2
        except Exception:
            pass
